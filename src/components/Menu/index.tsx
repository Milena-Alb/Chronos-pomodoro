...
export function Menu() {
    const [theme, setTheme] = useState<AvailableThemes>(() => {
        const storageTheme = localStorage.getItem('theme') as AvailableThemes || 'dark'; return storageTheme;
    });
    ...
    function handleThemeChange (event: React.MouseEvent<HTMLAnchorElement, MouseEvent>){
        event.preventDefault(); //Não segue o link, não segue o padrão
        setTheme(prevTheme => {
    ...
    return (
        <nav className={styles.menu}>
            ...
            <RouterLink className={styles.menuLink} href="#" aria-label='Tema' title='Tema' onClick={handleThemeChange}>
                {nextThemeIcon[theme]}
            </RouterLink>
        </nav>
    );
